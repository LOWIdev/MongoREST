...
const Tour = require('../model/GuidedTours');
...
const deleteTour = async (req, res) => {
    ...
    if (!tour) {
        return res.status(204).json({ "message": `No tour matches ID ${req.body.id}.` });
    }
...
const getTour = async (req, res) => {
    ...
    const user = await Tour.findOne({ _id: req.params.id }).exec();
